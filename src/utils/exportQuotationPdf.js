...
function buildTemperaturePageHtml({ temperature, groupProducts, template, channel, pageInfo, meta }) {
  ...
  const productRowsHtml = orderedSubs.map(subName => {
    const rows = bySub[subName].map(p => {
      ...
      return `
        <tr>
          ...
          <td style="padding:8px 6px;border:1px solid ${COLOR.borderGray};font-size:11px;text-align:center;vertical-align:middle;width:65px;color:${COLOR.textMuted};">
            ${p.validityPeriod ? escapeHtml(p.validityPeriod) : `<span style="color:${COLOR.textLight};">—</span>`}
          </td>
          ...
          <td style="padding:8px 6px;border:1px solid ${COLOR.borderGray};font-size:11px;text-align:center;vertical-align:middle;width:80px;">
            ${pageUrl ? `<span style="color:${COLOR.link};font-weight:600;">網頁連結</span>` : `<span style="color:${COLOR.textLight};">—</span>`}
          </td>
    ...
    return `
      <tr>
        <td colspan="8" style="padding:4px 14px;border:1px solid ${COLOR.borderGray};background:${COLOR.brandSoft};color:${COLOR.brand};font-size:11px;font-weight:700;">
          ${escapeHtml(subName)}
        </td>
      </tr>
      ${rows}
  ...
  const summaryHtml = isLastPage ? `
    <tr>
      <td colspan="8" style="padding:6px 14px;border:1px solid ${COLOR.borderGray};background:${COLOR.bgHeader};font-size:10px;color:${COLOR.textMuted};text-align:right;">
  ...
  ` : `
    <tr>
      <td colspan="8" style="padding:6px 14px;font-size:10px;color:${COLOR.textMuted};text-align:right;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        <thead>
          <tr>
            ...
            <th style="padding:6px 6px;border:1px solid ${COLOR.borderGray};background:${COLOR.bgHeader};font-size:11px;font-weight:700;color:${COLOR.textMuted};text-align:center;width:65px;">保存期限</th>
            ...
            <th style="padding:6px 6px;border:1px solid ${COLOR.borderGray};background:${COLOR.bgHeader};font-size:11px;font-weight:700;color:${COLOR.textMuted};text-align:center;width:80px;">產品介紹</th>
